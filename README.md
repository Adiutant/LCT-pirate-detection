...
### Обучение моделей
#### ViT
Может быть обучен с помощью ноутбука [вставить ноут], но не обучался, использовалась pretrained версия.
#### PANNS
Не обучался, использовалась pretrained версия.
#### Сиамская модель
Обучалась с помощью ноутбука [вставить ноут], датасет был подготовлен следующим образом - в интервалах сходства были взяты пары соответствующих посекундных кадров с лейблом 1, и пары несоответствующих кадров с лейблом 0. Такой датасет может быть использован для обучения ViT.
