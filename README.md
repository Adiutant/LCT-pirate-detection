...
| Требования к памяти  | vRAM: 24 Gb (inference) | vRAM: <=3 Gb (inference) | vRAM: <=3 Gb (inference) |
| Скорость работы  | Быстро | Быстро | Быстро |
...
### Вывод
Изначально мы решили использовать предобученные модели ViT и PANNS и сравнивать эмбеддинги по косинусному расстоянию. Такой подход подошел для стадии PoC, модель неплохо искала интервалы. Потом мы попробовали использовать сиамскую модель для создания эмбеддингов, но это привело к ухудшению результата, возможно потому, что размерность эмбеддингов такой модели слишком мала, а так же в такой модели нет механизма внимания. На последней итерации мы обучили ViT на подготовленном датасете и добились на нем метрики в 0.98 accuracy. То есть модель определяет соответствие кадра зависимости кадру лицензионного видео почти в 100% случаев. Такое высокое качество модели позволило сильно повысить итоговое качество работы всей поисковой системы. Время для индексации часового видео не превышало 2 минут. Время для поиска по базе данных может увеличиваться с увеличением индексированных видео, но на тестовых данных не превышает 1 минуты.
